...
bool rpg_options_update_indicator(game_object_t *object, scene_t *scene)
{
...
game_object_t *rpg_options_create_indicator(game_object_t *last, char *path, sfVector2f pos)
{
    game_object_t *object = create_game_object(last, path, pos, DECOR);

    object->update = &rpg_options_update_indicator;
    object->type = DECOR;
