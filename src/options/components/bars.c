...
bool rpg_options_update_bar(game_object_t *bar, scene_t *scene)
{
...
void rpg_options_bar_function(game_object_t *object, void *pt)
{
    object->state = SELECTED;
}

game_object_t *rpg_options_create_bar(game_object_t *last, char *path, sfVector2f pos)
{
    game_object_t *object = create_game_object(last, path, pos, BAR);

    object->state = UNSELECTED;
    object->callback = &rpg_options_bar_function;
    object->update = &rpg_options_update_bar;
